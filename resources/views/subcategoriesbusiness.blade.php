...
@section('title', 'Subcategorias de negocios')
    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title mb-0">Lista de subcategorías de negocios</h4>
            </div><!-- end card header -->
                ...
                <div class="listjs-table" id="customerList">
                    ...
                    <div class="table-responsive table-card mt-3 mb-1">
                        <table class="table align-middle table-nowrap" id="customerTable">
                            ...
                            <tbody>
                                @foreach($subcategoriesbusiness as $subcategory)
                                <tr>
                                    <td></td>
                                    <td>{{ $subcategory->id }}</td>
                                    <td>{{ $subcategory->name }}</td>
                                    <td>{{ $subcategory->description }}</td>
                                    <td>{{ $subcategory->businessCategory->name }}</td> <!-- Cambiado de 'category' a 'businessCategory' -->
                        ...
                        @if($subcategoriesbusiness->isEmpty())
                        <div class="noresult">
...
<div class="modal fade" id="showModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="{{ route('subcategoriesbusiness.store') }}" method="POST"> <!-- Cambiado a 'subcategoriesbusiness.store' -->
                @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <select name="business_category_id" id="business_category_id" class="form-control" required>
                            ...
                            @foreach($businesscategories as $businesscategory)
                                <option value="{{$businesscategory->id }}">{{ $businesscategory->name }} - {{ $businesscategory->description }}</option>
                            @endforeach
                        </select>
...
<div class="modal fade" id="editModal{{ $subcategory->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form method="POST" action="{{ route('subcategoriesbusiness.update', $subcategory->id) }}"> <!-- Cambiado a 'subcategoriesbusiness.update' -->
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="business_category_id" id="business_category_id" class="form-control">
                            @foreach($businesscategories as $businesscategory) <!-- Cambiado a 'businesscategory' -->
                            <option value="{{ $businesscategory->id }}" {{ $subcategory->business_category_id == $businesscategory->id ? 'selected' : '' }}>{{ $businesscategory->name }} - {{ $businesscategory->description }}</option> <!-- Cambiado a 'businesscategory' -->
                            @endforeach
                        </select>
                    </div>
                </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="d-flex gap-2 justify-content-center mt-4 mb-2">
                    ...
                    <form id="delete-subcategory-form{{ $subcategory->id }}" action="{{ route('subcategoriesbusiness.destroy', $subcategory->id) }}" method="POST"> <!-- Cambiado a 'subcategoriesbusiness.destroy' -->
                        @csrf
                        @method('DELETE')
